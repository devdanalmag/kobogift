...
function InfoRow({
  ...
  return (
    <div className="flex gap-3">
      <span className="mt-0.5 text-lg leading-none text-orange-400 flex items-center justify-center">{icon}</span>
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-0 left-1/2 z-[59] w-full max-w-[520px] -translate-x-1/2 overflow-y-auto rounded-t-3xl border border-orange-500/20 bg-[#0c121e] px-5 pb-10 pt-5 shadow-2xl"
            style={{ maxHeight: "88vh" }}
          >
            {/* Drag handle */}
            <div className="mx-auto mb-4 h-1.5 w-12 rounded-full bg-orange-500/30" />

            {/* Header */}
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-base font-bold text-white flex items-center gap-2">
                <LuGift className="w-5 h-5 text-orange-400" /> How KoboGift works
              </h2>
              <button
                type="button"
                onClick={() => setOpen(false)}
            ...
            <div className="space-y-5">
              <InfoRow icon={<LuBanknote className="w-5 h-5" />} title="Sending a gift">
                One Circle confirmation runs a <span className="text-orange-300 font-medium">batch transaction</span>:
              ...
              <InfoRow icon={<LuLink className="w-5 h-5" />} title="How the link works">
                The URL path stores only a <span className="text-orange-300 font-medium">hash</span>.
                The secret that unlocks the funds lives after the{" "}
                <code className="rounded bg-orange-950/60 border border-orange-500/30 px-1.5 py-0.5 text-xs text-orange-300">#</code>{" "}
                — it never reaches the server, so only the person with the link can claim.
              </InfoRow>

              <div className="h-px bg-white/8" />

              <InfoRow icon={<LuGift className="w-5 h-5" />} title="Claiming a gift">
                Open the gift link and tap <span className="text-orange-300 font-medium">Unwrap</span>.
              ...
              <InfoRow icon={<LuFuel className="w-5 h-5" />} title="Who pays gas">
              ...
              <InfoRow icon={<LuSearch className="w-5 h-5" />} title="Verifying a transaction">
              ...
              <InfoRow icon={<LuWallet className="w-5 h-5" />} title="Viewing your wallet">
              ...
              <InfoRow icon={<LuLock className="w-5 h-5" />} title="Sign-in & navigation">
